...
- **xlsx**: 값·수식·병합·열너비·다른 시트 유지. 단 **무료 SheetJS 는 저장 시
  셀 색/글꼴 등 서식을 제외**한다.
- **docx**: mammoth 로 단순 HTML 편집 → html-to-docx 로 저장. 기본 요소(문단·
  굵게·표) 위주. 원본의 정교한 서식은 일부 손실. **워드 저장은 앱(.exe) 전용**
  (브라우저는 편집만, 저장은 앱 안내). 최종 서식 중요 문서는 워드에서 마무리.
