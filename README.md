...
**설치 & 사용**
- 설치 파일 실행 → **관리자 권한 없이 내 계정에만 설치**(폐쇄망/권한제한 PC 적합)
- 설치되면 `.docx`/`.xlsx` **아이콘이 문서뷰어 아이콘으로 바뀌고**, 더블클릭 시
  이 앱으로 열립니다. (수동으로 "연결 프로그램" 지정 불필요)
- 바탕화면·시작메뉴 바로가기가 생성됩니다.

> 서명되지 않은 설치 파일이라 최초 실행 시 SmartScreen 경고가 나오면
> "추가 정보 → 실행"을 누르세요. (파일 우클릭 → 속성 → **차단 해제** 후 실행도 가능)

### 버전
현재 **v1.1.0**. 버전은 `package.json` 의 `version` 으로 관리하며, 설치 파일명과
